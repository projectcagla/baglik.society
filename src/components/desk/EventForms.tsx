  ...
  return (
    <form action={action} className={styles.grid}>
          ...
          {films.map((f) => (
            <label key={f.id}>
              <input
                ...
                name="film_ids"
                value={f.id}
                defaultChecked={values.film_ids.includes(f.id)}
              />{' '}
              {f.label}
            </label>
          ))}
      ...
      <div className={styles.cols2}>
        <Text
          name="location_public_note"
          label="genel konum metni"
          defaultValue={values.location_public_note}
          required
          hint="konum açılana kadar davetlilerin gördüğü cümle"
        />
        <Text
          name="status_note"
          label="durum notu"
          defaultValue={values.status_note}
          hint="erteleme/iptal açıklaması"
        />
      </div>
      <div className={styles.cols2}>
        ...
        <Check
          ...
          label="“geliyorum” diyenlerin adlarını davetlilere göster"
          defaultChecked={values.guest_list_visible}
        />
      </div>
  ...
  return (
    <form action={action} className={styles.grid}>
      ...
      <Text
        name="location_text"
        label="gerçek konum"
        defaultValue={values.location_text}
        hint="bilinmiyorsa boş bırak — sahte adres girme"
      />
      <div className={styles.cols2}>
        <Text
          name="location_url"
          ...
          defaultValue={values.location_url}
        />
      ...
      <div className={styles.cols3}>
        <Text
          name="release_at"
          type="datetime-local"
          label="açılış zamanı (istanbul)"
          defaultValue={values.release_at}
          hint="bu andan sonra uygun davetliler görür"
        />
        <Select
          ...
          options={
            [
              ['katilanlar', '“geliyorum” diyenler'],
              ['davetliler', 'tüm davetliler'],
            ] as const
          }
        ...
        <Check
          ...
          label="bildirim e-postasına adresi ekle"
          defaultChecked={values.include_in_email}
        />
      </div>
      <Area
        name="admin_note"
        label="yönetici notu (yalnızca yöneticiler)"
        defaultValue={values.admin_note}
        rows={3}
      />
      <div className={ui.row}>
        <button type="submit" className={`${ui.button} ${ui.primary}`} disabled={pending}>
