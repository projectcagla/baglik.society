...
import { createEntry, listSharedEntries, shareEntry, listOwnEntries, addContribution } from '@/server/dal/journal';
...
describe('anonymous (baglik_app without identity)', () => {
  it('reads nothing from any content table', async () => {
    await asAnonymous(async (tx) => {
      for (const table of ['films', 'resources', 'questions', 'screening_notes', 'events', 'event_films', 'event_invitees', 'event_private', 'members', 'journal_entries', 'contributions', 'audit_logs', 'notification_deliveries']) {
  ...
  it('cannot touch the private schema at all', async () => {
    await expect(asAnonymous((tx) => tx`select * from private.sessions`)).rejects.toThrow(/permission denied/);
    await expect(asAnonymous((tx) => tx`select * from private.credentials`)).rejects.toThrow(/permission denied/);
  });

  it('cannot write', async () => {
    await expect(asAnonymous((tx) => tx`insert into films (slug, title) values ('x-film', 'x')`)).rejects.toThrow();
  });

  it('gets "yok" from the location function', async () => {
    const id = await eventTwo();
    const [row] = await asAnonymous((tx) => tx<{ state: string; location_text: string | null }[]>`select * from app.event_location(${id})`);
...
describe('members, films and the two layers', () => {
  ...
  it('seed: both real films visible to a member, in club order', async () => {
    ...
    expect(canavar).toMatchObject({ title: 'canavar', title_original: 'Monster', director: 'Hirokazu Kore-eda', year: 2023, status: 'yaklasiyor' });
  });
  ...
  it('canavar pre-reading: articles, interviews and listening — no film recommendations, no books', async () => {
    ...
    expect(detail!.before.map((r) => r.url)).toContain('https://www.bfi.org.uk/features/where-begin-hirokazu-koreeda');
  ...
  it('draft films are invisible to members, visible to editors', async () => {
    await asMember({ memberId: editor.id, mfa: false }, (tx) => tx`insert into films (slug, title) values ('999-taslak', 'taslak film')`);
  ...
  it('the after layer is withheld by the database until published', async () => {
    ...
    await expect(addContribution(member, { filmId, questionId: null, parentId: null, body: 'x', attribution: 'isimli' })).rejects.toThrow();
...
describe('events and invitations', () => {
  it('only invited members see the night', async () => {
    ...
    expect((await getEventByNumber(invited, 2))?.event.starts_at.toISOString()).toBe('2026-09-27T16:30:00.000Z');
  ...
  it('members cannot write RSVPs directly, only through app.set_rsvp', async () => {
    ...
    const updated = await asMember({ memberId: m.id, mfa: false }, (tx) => tx`update event_invitees set rsvp = 'geliyorum', status = 'davetli' where member_id = ${m.id} returning 1`);
    expect(updated).toHaveLength(0);
    await setRsvp(m, eventId, 'geliyorum', 'biraz geç kalabilirim');
    expect((await getEventByNumber(m, 2))?.invite).toMatchObject({ rsvp: 'geliyorum', rsvp_note: 'biraz geç kalabilirim' });
  });
  ...
  it('refuses RSVP after the deadline and for non-invitees', async () => {
    ...
    await invite(eventId, m.id);
    await asSystem((tx) => tx`update events set rsvp_deadline = now() - interval '1 minute' where id = ${eventId}`);
  ...
  it('invitee lists are not visible to other members', async () => {
    ...
    const rows = await asMember({ memberId: a.id, mfa: false }, (tx) => tx`select member_id from event_invitees where event_id = ${eventId}`);
    expect(rows.map((r) => r.member_id)).toEqual([a.id]);
    const guests = await asMember({ memberId: a.id, mfa: false }, (tx) => tx`select * from app.event_guest_list(${eventId})`);
...
describe('journal privacy', () => {
  it('private notes stay with their owner — even admins with MFA cannot read them', async () => {
    ...
    for (const v of [b, admin]) {
      const rows = await asMember({ memberId: v.id, mfa: v.mfaFresh }, (tx) => tx`select body from journal_entries where body = 'GIZLI-NOT'`);
    ...
    const edited = await asMember({ memberId: b.id, mfa: false }, (tx) => tx`update journal_entries set body = 'x' where body = 'GIZLI-NOT' returning 1`);
...
describe('roles and the member directory', () => {
  it('members see only themselves; admins need MFA to see everyone', async () => {
    const m = viewerFor(await makeMember('member'), 'member');
    const rows = await asMember({ memberId: m.id, mfa: false }, (tx) => tx<{ id: string }[]>`select id from members`);
  ...
  it('editors cannot create members or read locations', async () => {
    ...
    await expect(createMember(editor, { display_name: 'x', email: null, role: 'member' })).rejects.toThrow();
    const rows = await asMember({ memberId: editor.id, mfa: false }, (tx) => tx`select * from event_private`);
    expect(rows).toHaveLength(0);
    await expect(saveEventPrivate(editor, await eventTwo(), { location_text: 'x', location_url: null, location_directions: null, release_at: null, release_audience: 'katilanlar', include_in_email: false, admin_note: null })).rejects.toThrow(/not allowed/);
  });

  it('only an owner can create admins', async () => {
    const admin = viewerFor(await makeMember('admin'), 'admin', true);
    const owner = viewerFor(await makeMember('owner'), 'owner', true);
    await expect(createMember(admin, { display_name: 'yeni yönetici', email: null, role: 'admin' })).rejects.toThrow(/only an owner/);
    await expect(createMember(owner, { display_name: 'yeni yönetici', email: null, role: 'admin' })).resolves.toBeTruthy();
  });
