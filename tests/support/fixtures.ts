...
export async function makeMember(
  role: Role = 'member',
  opts: { email?: string | null; status?: string } = {},
) {
  n += 1;
...
export function viewerFor(
  m: { id: string; name: string; email: string | null },
  role: Role,
  mfa = false,
): Viewer {
...
export async function film(slug: string) {
  const [f] = await asSystem(
    (tx) => tx<{ id: string }[]>`select id from films where slug = ${slug}`,
  );
  return f!.id;
}
...
export async function invite(eventId: string, memberId: string) {
  await asSystem(
    (tx) =>
      tx`insert into event_invitees (event_id, member_id) values (${eventId}, ${memberId}) on conflict do nothing`,
  );
}
...
export async function setPrivate(
  ...
  await asSystem(async (tx) => {
    if ('location_text' in patch)
      await tx`update event_private set location_text = ${patch.location_text ?? null} where event_id = ${eventId}`;
    if ('release_at' in patch)
      await tx`update event_private set release_at = ${patch.release_at ?? null} where event_id = ${eventId}`;
    if ('released_at' in patch)
      await tx`update event_private set released_at = ${patch.released_at ?? null} where event_id = ${eventId}`;
    if (patch.release_audience)
      await tx`update event_private set release_audience = ${patch.release_audience} where event_id = ${eventId}`;
  });
}
